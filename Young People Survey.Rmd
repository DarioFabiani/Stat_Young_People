...
```{r Ward method based clustering and visualization, fig.height=13, fig.align="center", message=FALSE}
library(factoextra)
library(ggplot2)
library(gridExtra)
...
```
```{r PCA Visualization (Spending Habits), fig.align="center", message=FALSE}
library(gridExtra)
library(factoextra)
library(ggplot2)
...
```{r Phobias correlation Visualization}
pal <- colorRampPalette(c("#FC4E07", "#E7B800", "#00AFBB"))
...
```
```{r PCA Phobias Visualization, fig.align="center"}
...
```
## Regression  
Now that we have our clusters that tries to describe different types of personalities, Principal Components that try to summaries with two variables the original seven features related to spending habits, we will try to perform a multiple regression to start responding our research contraint: *"It is possible to try predicting spending habits on the basis of different types of personalities?"*.
...
par(mfrow=c(2,2))
plot(spend.summary$rss, type="l")

plot(spend.summary$adjr2, type="l")
points(which.max(spend.summary$adjr2), spend.summary$adjr2[which.max(spend.summary$adjr2)], col="#E7B800", cex=2, pch=20)

plot(spend.summary$cp, type="l")
points(which.min(spend.summary$cp), spend.summary$cp[which.min(spend.summary$cp)], col="#E7B800", cex=2, pch=20)

plot(spend.summary$bic, type="l")
points(which.min(spend.summary$bic), spend.summary$bic[which.min(spend.summary$bic)], col="#E7B800", cex=2, pch=20)
```
...
par(mfrow=c(2,2))
plot(phob.summary$rss, type="l")

plot(phob.summary$adjr2, type="l")
points(which.max(phob.summary$adjr2), phob.summary$adjr2[which.max(phob.summary$adjr2)], col="#E7B800", cex=2, pch=20)

plot(phob.summary$cp, type="l")
points(which.min(phob.summary$cp), phob.summary$cp[which.min(phob.summary$cp)], col="#E7B800", cex=2, pch=20)

plot(phob.summary$bic, type="l")
points(which.min(phob.summary$bic), phob.summary$bic[which.min(phob.summary$bic)], col="#E7B800", cex=2, pch=20)
```
```{r}

plot(regfit.phob, scale="r2")
plot(regfit.phob, scale="adjr2")
plot(regfit.phob, scale="Cp")
plot(regfit.phob, scale="bic")
